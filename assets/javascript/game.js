var gameObject = {
	...
	wireArray: ["Baltimore", "Hamsterdam", "Stinger Bell", "Bubbles", "Omar Comin", "McNulty"],
	randomWord: "",
	wireLetters:[],
	...
	generateWord: function(){
		...
		this.randomWord = this.wireArray[random_num];
		this.wireLetters = this.randomWord.split("");

		//Shows that a randomly chosen band name was converted into an array containing each of its letters.
		console.log(this.randomWord + " " + this.wireLetters);
	...
	checkMatch: function(){
		...
		//Loop for the word length amount of times.
		//If the guessed letter is equal to the the wire's letter at a given index, the counter variable counts up one.
		for (var i=0; i < this.wireLetters.length; i++){
			if (this.currentLetter == this.wireLetters[i]){
				matchCounter++;
			}
		}
	...
	revealWire: function(){
		...
		//For the number of letters in the wire's name, fill the displayed guesses with an underscore.
		if (this.correctGuesses.length == 0){
			...
			//For the length of the wire's name,
			for (var i=0; i<this.wireLetters.length; i++){
				//If the displayed guess is not the same as wireLetters at index i,
				if (this.correctGuessesInOrder[i] != this.wireLetters[i]){
					...
					for (var j=0; j<this.correctGuesses.length; j++){
						...
						if (this.correctGuesses[j] == this.wireLetters[i]){
							this.correctGuessesInOrder[i] = this.wireLetters[i];
						}
	...
	checkProgress: function(){
		...
		if (counter == this.wireLetters.length){
			alert("You win");
			this.winCount++;
			this.generateWord();
		}
